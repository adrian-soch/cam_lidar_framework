...
class LidarNode(Node):
    ...
    def __init__(self):
        ...
        self.camera_mat, self.dist = get_calib_from_file(
            CAM_CAL_PATH, ['camera_matrix', 'distortion_coefficients'], intrinsic=True)
        ...
        # Must be 4x4
        self.ld2ls_mat = np.vstack(
            [self.ld2ls_mat, np.array([0, 0, 0, 1], dtype=np.float32)])
    ...
    def worker(self):
        while True:
            ...
            newcameramtx, roi = cv2.getOptimalNewCameraMatrix(
                self.camera_mat, self.dist, (img.shape[1], img.shape[0]), 1, (img.shape[1], img.shape[0]))
            ...
            self.get_logger().info('Q '+str(t2-t1)+' PC '+str(t3-t2) +
                                   ' Proj '+str(t4-t3)+' Pub '+str(t5-t4))
