...
class LidarNode(Node):
    ...
    def __init__(self):
        ...
        if USE_SENSOR_QOS:
            self.img = mf.Subscriber(self, Image, "/image", qos_profile=qos.qos_profile_sensor_data)
            self.pc2 = mf.Subscriber(self, PointCloud2, "/points", qos_profile=qos.qos_profile_sensor_data)
        else:
            self.img = mf.Subscriber(self, Image, "/image")
            self.pc2 = mf.Subscriber(self, PointCloud2, "/points")

        # Wait 0.1 seconds, and store a queue up to 6 while trying to match msgs
        self.ts = mf.ApproximateTimeSynchronizer([self.img, self.pc2], queue_size=10, slop=0.1)
    ...
    def callback(self, img_msg, pc2_msg):
        ...
        # (3*3) @ (3*N) = (3*N)
        px_proj = (self.camera_mat @ temp)[0]
        ...
        for idx,i in enumerate(px_proj):
            ...
            cv2.circle(img, np.int32(i), 2, color, -1)
